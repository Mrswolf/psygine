...
def trca_kernel(X, cov_estimator='cov', n_jobs=None):
    r"""TRCA kernel.
    ...
    Q = covariances(
        X, estimator=cov_estimator, n_jobs=n_jobs)
    Q = np.mean(Q, axis=0)
    S = S_bar - Q
    D, W = eigh(S, Q)
    D, W = D[::-1], W[:,::-1]
...
class TRCA(BaseEstimator, TransformerMixin, ClassifierMixin):
    ...
    def fit(self, X, y):
        ...
        _, Ws, Ts = zip(*Parallel(n_jobs=self.n_jobs)(
            delayed(partial(trca_kernel, cov_estimator=self.cov_estimator, n_jobs=1))(X[y==label]) for label in self.classes_))
...
class FBTRCA(FilterBank, ClassifierMixin):
    ...
    def __init__(self,
        ...
        super().__init__(
            TRCA(n_components=n_components, cov_estimator=cov_estimator, ensemble=ensemble, n_jobs=1),
            filterbank,
            concat=False,
            n_jobs=n_jobs)
